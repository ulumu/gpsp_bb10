...
//#define __FUNCTION_DEBUG__
//#define __NO_MACRO_BUILDER__
...
#ifdef __NO_MACRO_BUILDER__
static u32 opcode = 0;
static u32 last_opcode;
static u32 condition;
static u32 last_condition;
static u32 block_start_pc;
static u32 block_end_pc;
static u32 block_exit_position = 0;
static s32 block_data_position = 0;
static u32 external_block_exit_position = 0;
static u32 branch_target;
static u32 cycle_count = 0;
static u8 *translation_target;
static u8 *backpatch_address = NULL;
static u8 *translation_ptr = NULL;
static u8 *translation_cache_limit = NULL;
static block_exit_type external_block_exits[MAX_EXITS];
static u32 pc_region;
static u32 new_pc_region;
static u8 *pc_address_block;
#endif
...
void translate_arm_instruction(u32 *in_pc, u32 *in_flagStatus)
{
	u32 pc          = *in_pc;
	u32 flag_status = *in_flagStatus;
...
void translate_thumb_instruction(u32 *in_pc, u32 *in_flag_status)
{
	  u32 pc = *in_pc;

	  generate_block_extra_vars_thumb();

	  u32 flag_status = block_data[block_data_position].flag_data;
	  check_pc_region(pc);
	  last_opcode = opcode;
	  opcode = address16(pc_address_block, (pc & 0x7FFF));
...
u8 function_cc *block_lookup_address_arm(u32 pc)
{
  ...
  switch(pc >> 24)
  {
    case 0x0:
      ...
      block_lookup_translate(arm, bios, 0);
      if(translation_recursion_level == 0)
        bios_region_read_allow();
      break;
    ...
    case 0x2:
      ...
      block_lookup_translate(arm, ram, 1);
      if(translation_recursion_level == 0)
    ...
    case 0x6:
    	SLOG("Lookup address hit 0x06000000 region (VRAM region), pc=0x%X!!", pc);
      location = (u16 *)(vram + (pc & 0x7FFF));
      block_lookup_translate(arm, ram, 1);
      if(translation_recursion_level == 0)
        bios_region_read_protect();
      break;
    ...
    default:
      ...
      if(translation_recursion_level == 0)
      {
    	  SLOG("bad jump %x (%x) (%x)n", pc, reg[REG_PC], last_instruction);
    	  quit();
      }
      block_address = (u8 *)(-1);
      break;
...
u8 function_cc *block_lookup_address_thumb(u32 pc)
{
  ...
  switch(pc >> 24)
  {
    case 0x0:
      bios_region_read_allow();
      location = (u16 *)(bios_rom + pc + 0x4000);
      block_lookup_translate(thumb, bios, 0);
      if(translation_recursion_level == 0)
        bios_region_read_allow();
      break;
    ...
    case 0x2:
      location = (u16 *)(ewram + (pc & 0x7FFF) + ((pc & 0x38000) * 2));
      block_lookup_translate(thumb, ram, 1);
      if(translation_recursion_level == 0)
        bios_region_read_protect();
      break;
    ...
    case 0x8 ... 0xD:
    {
      ...
      if(block_ptr == NULL)
      {
        ...
        *block_ptr_address = (u32 *)rom_translation_ptr;
        rom_translation_ptr += 8;
        block_address = rom_translation_ptr + block_prologue_size;
        block_lookup_translate_thumb(rom, 0);
        translation_recursion_level--;
...
u8 function_cc *block_lookup_address_dual(u32 pc)
{
  ...
  switch(pc >> 24)
  {
    case 0x0:
      bios_region_read_allow();
      location = (u16 *)(bios_rom + pc + 0x4000);
      block_lookup_translate(dual, bios, 0);
      if(translation_recursion_level == 0)
        bios_region_read_allow();
      break;
    ...
    case 0x2:
      location = (u16 *)(ewram + (pc & 0x7FFF) + ((pc & 0x38000) * 2));
      block_lookup_translate(dual, ram, 1);
      if(translation_recursion_level == 0)
        bios_region_read_protect();
      break;
    ...
    case 0x8 ... 0xD:
    {
      ...
      if(block_ptr == NULL)
      {
        ...
        *block_ptr_address = (u32 *)rom_translation_ptr;
        rom_translation_ptr += 8;
        block_address = rom_translation_ptr + block_prologue_size;
        block_lookup_translate_dual(rom, 0);
        translation_recursion_level--;
  ...
  {                                                                           \
    ...
    {                                                                         \
      ...
      {                                                                       \
        ...
        type##_branch_target();                                               \
        DBGLOG("opcode br/bx 0x%X", branch_target);                           \
      ...
      if(block_exit_position == MAX_EXITS) {                                  \
    	  DBGLOG("Reaching MAX capacity for block_exits");                      \
          break;                                                              \
      }                                                                       \
    }                                                                         \
    else                                                                      \
...
#ifdef __NO_MACRO_BUILDER__
void scan_block_arm(int smc_write_op)
{
	int i;
...
//  __label__ block_end;
  ...
  {
    check_pc_region(block_end_pc);
    if (smc_write_op) { smc_write_arm_yes(); }
    arm_load_opcode();
    arm_flag_status();
    ...
    if(arm_exit_point)
    {
      ...
      arm_set_condition(condition | 0x10);

      /* Only unconditional branches can end the block. */
      if(arm_opcode_unconditional_branch)
      {
        /* Check to see if any prior block exits branch after here,
           if so don't end the block. Starts from the top and works
           down because the most recent branch is most likely to
...
{                                                                             \
  ...
  s32 i;                                                                      \
  u32 flag_status;                                                            \
  block_exit_type external_block_exits[EXT_MAX_EXITS];                        \
  type##_fix_pc();                                                            \
  ...
  {                                                                           \
    ...
    {                                                                         \
      ...
      if (external_block_exit_position >= EXT_MAX_EXITS) {                    \
    	  DBGLOG("Not enough space to hold external branch!!!");                \
    	  DBGLOG("This is bad!!!!");                                            \
    	  DBGLOG("Total branches=%d vs %d", block_exit_position,EXT_MAX_EXITS); \
    	  break;                                                              \
      }                                                                       \
    }                                                                         \
  }                                                                           \
		...
		translation_region, u32 smc_enable)
{
	...
	case TRANSLATION_REGION_RAM:
		...
		SLOG("**** translation_ptr = ram_translation_ptr [0x%X]",  (int)ram_translation_ptr);
		translation_ptr = ram_translation_ptr;
		translation_cache_limit =
				ram_translation_cache + RAM_TRANSLATION_CACHE_SIZE -
				TRANSLATION_CACHE_LIMIT_THRESHOLD;
	...
	case TRANSLATION_REGION_ROM:
		SLOG("**** translation_ptr = rom_translation_ptr [0x%X]",  (int)rom_translation_ptr);
		translation_ptr = rom_translation_ptr;
		translation_cache_limit =
				rom_translation_cache + ROM_TRANSLATION_CACHE_SIZE -
				TRANSLATION_CACHE_LIMIT_THRESHOLD;
	...
	case TRANSLATION_REGION_BIOS:
		SLOG("**** translation_ptr = bios_translation_ptr [0x%X]",  (int)bios_translation_ptr);
		translation_ptr = bios_translation_ptr;
		translation_cache_limit = bios_translation_cache +
				BIOS_TRANSLATION_CACHE_SIZE;
		break;
	...
	while(pc != block_end_pc)
	{
		...
		translate_arm_instruction(&pc, &flag_status);
#else
		generate_block_extra_vars_arm();
...
#ifdef __FUNCTION_DEBUG__
    translate_thumb_instruction(&pc, &flag_status);
#else
    generate_block_extra_vars_thumb();
...
#if !defined PC_BUILD && !defined __X86__
