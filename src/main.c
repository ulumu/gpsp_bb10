...
#if defined RPI_BUILD || defined BB10_BUILD
...
int main(int argc, char *argv[])
{
  ...
  char path[] = "/accounts/1000/shared/misc/gbaemu/savegames";
  ...
  sprintf(bios_filename, "%s" PATH_SEPARATOR "%s", "/accounts/1000/shared/misc/roms/gba", "gba_bios.bin");
  ret = load_bios(bios_filename);
  if (ret != 0) {
	  // cannot find bios from phone, try external SD card
	  sprintf(bios_filename, "%s" PATH_SEPARATOR "%s", "/accounts/1000/removable/sdcard/misc/roms/gba", "gba_bios.bin");
	  ret = load_bios(bios_filename);
  }
  ...
  if (ret != 0)
  {
      bbShowAlert(
              "GBA Bios Not Found",
              "gpsp_bb requires a Gameboy Advance BIOS image to run. Make sure copy gba_bios.bin to /misc/roms/gba");
  ...
  if(bios_rom[0] != 0x18)
  {
      bbShowNotification("An Incorrect BIOS image is provided!! gpsp_bb will continue run with RISK!!");
  }
  ...
  // Show BB10 dialog for ROM file selection
  loadRomDialog(load_filename);

  if ( load_filename[0] )
  {
	  if(load_gamepak(load_filename) == -1)
	  {
		  SLOG("Failed to load gamepak %s, exiting.\n", load_filename);
		  quit();
		  exit(-1);
	  }
...
void trigger_ext_event()
{
  ...
  switch(event_number)
  {
    ...
    case 5:
    {
      ...
      SLOG("Benchmark results (%d frames): \n", event_cycles_trigger);
      for(i = 0; i < 8; i++)
      {
        SLOG("   %s: %d ms (%f ms per frame)\n",
...
/*          SLOG("%d gbc audio updates\n", gbc_update_count);
    ...
    if(irq_raised)
      raise_interrupt((irq_type)irq_raised);
...
void synchronize()
{
  ...
  {
#if defined(PND_BUILD) || defined(RPI_BUILD) || defined(BB10_BUILD)
    fb_wait_vsync();
...
#if !defined(GP2X_BUILD) && !defined(PND_BUILD) && !defined(RPI_BUILD) && !defined(BB10_BUILD)
  char char_buffer[64];
...
void make_rpath(char *buff, size_t size, const char *ext)
{
  ...
  snprintf(buff, size, "%s/%s.gba", main_path, p);
  p = strrchr(buff, '.');
  if (p != NULL)
    strcpy(p, ext);
}
...
void set_clock_speed()
{
  ...
  {
    SLOG("about to set CPU clock to %iMHz\n", clock_speed);
  #ifdef PSP_BUILD
