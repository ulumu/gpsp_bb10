...
#define write32(value)                                                        \
	DBGLOG("addr[0x%X] = 0x%X", (int)translation_ptr, (int)value);              \
...
#define generate_branch_patch_conditional(dest, offset)                       \
	DBGLOG("*** generate_branch_patch_conditional: dest=0x%X, offset=0x%X", (int)dest, (int)offset); \
...
#define generate_branch_no_cycle_update(writeback_location, new_pc, mode)     \
  ...
  {                                                                           \
	  DBGLOG("generate_branch_idle_eliminate: writeback_loc:0x%X, newPC:0x%X, mode:%s", (int)writeback_location, (int)new_pc, #mode); \
    generate_branch_idle_eliminate(writeback_location, new_pc, mode);         \
  }                                                                           \
  else                                                                        \
  {                                                                           \
	  DBGLOG("generate_branch_update: writeback_loc:0x%X, newPC:0x%X, mode:%s", (int)writeback_location, (int)new_pc, #mode); \
...
{
  set_cpu_mode( (cpu_mode_type)((u32)cpu_modes[reg[REG_CPSR] & 0x1F]) );
...
{                                                                             \
	DBGLOG("**** thumb_conditional_branch: condition = %s", #condition); \
  generate_cycle_update();                                                    \
  generate_load_flags();                                                      \
