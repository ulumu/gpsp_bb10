...
#if 1 //ndef BB10_BUILD
u16  palette_ram[PALETTE_SIZE];
u16  oam_ram[OAM_SIZE];
u16  palette_ram_converted[PALETTE_SIZE];
u16  io_registers[IOMEM_SIZE];
u8   ewram[WORKRAM_SIZE];
u8   iwram[INTRAM_SIZE];
u8   vram[VRAM_SIZE];
u8   bios_rom[BIOS_SIZE];
...
u32 function_cc read_eeprom()
{
	...
	{
	case EEPROM_BASE_MODE:
		DBGLOG("EEPROM_BASE_MODE");
		value = 1;
		break;

	case EEPROM_READ_MODE:
		DBGLOG("EEPROM_READ_MODE");
	...
	case EEPROM_READ_HEADER_MODE:
		DBGLOG("EEPROM_READ_HEADER_MODE");
		value = 0;
		eeprom_counter++;
		if(eeprom_counter == 4)
		{
...
#define trigger_dma(dma_number)                                               \
  ...
  {                                                                           \
    ...
    {                                                                         \
      ...
      dma[dma_number].source_direction = (dma_increment_type)((value >>  7) & 0x03);                \
      ...
      {                                                                       \
        ...
        dma[dma_number].dest_direction = (dma_increment_type)((value >> 5) & 0x03);                 \
      }                                                                       \
                                                                              \
...
void function_cc write_backup(u32 address, u32 value)
{
	...
	if((address == 0x5555) && (flash_mode != FLASH_WRITE_MODE))
	{
		...
		if((address == 0x2AAA) && (value == 0x55) && (flash_command_position == 1))
		{
  ...
  if(file_check_valid(backup_file))
  {
    ...
    switch(backup_size)
    {
      case 0x200:
        ...
        eeprom_address_length = 0;
        eeprom_address = 0;
        eeprom_counter = 0;
        SLOG("Backup Type BACKUP_EEPROM, size EEPROM_512_BYTE");
        break;
      ...
      case 0x2000:
        ...
        eeprom_address = 0;
        eeprom_counter = 0;
        SLOG("Backup Type BACKUP_EEPROM, size EEPROM_8_KBYTE");
        break;

      case 0x8000:
        backup_type = BACKUP_SRAM;
        sram_size = SRAM_SIZE_32KB;
        SLOG("Backup Type BACKUP_SRAM, size SRAM_SIZE_32KB");
        break;
...
//        sram_size   = FLASH_SIZE_64KB;
        ...
        SLOG("Backup Type BACKUP_FLASH, size FLASH_SIZE_64KB");
        break;

      case 0x20000:
        backup_type = BACKUP_FLASH;
        flash_size = FLASH_SIZE_128KB;
        SLOG("Backup Type BACKUP_FLASH, size FLASH_SIZE_128KB");
        break;
    }
    return 1;
  }
  ...
  if(config_file)
  {
    while(fgets(current_line, 256, config_file))
    {
       ...
       != -1)
      {
        ...
        SLOG("Found match game config! GameName:%s, GameCode:%s, VendorCode:%s",
    ...
    SLOG("EOF @ Game Config Reaging");

    fclose(config_file);
  }

#ifndef PSP_BUILD
  SLOG("game config missing (%s)", config_path);
#endif
  return 0;
}
...
s32 load_gamepak(char *name)
{
  ...
  if(file_size != -1)
  {
    ...
    if (-1 == load_game_config(gamepak_title, gamepak_code, gamepak_maker))
    {
    	return -1;
    }
...
#define dma_print(src_op, dest_op, transfer_size, wb)                         \
  SLOG("dma from %x (%s) to %x (%s) for %x (%s) (%s) (%d) (pc %x)\n",       \
...
cpu_alert_type dma_transfer(dma_transfer_type *dma)
{
  ...
  DBGLOG("DMA (0x%X) --> (0x%X), len=%d", src_ptr, dest_ptr, length);
  ...
  if(dma->irq)
  {
    raise_interrupt((irq_type)((int)IRQ_DMA0 << dma->dma_channel));
...
void init_gamepak_buffer()
{
  ...
  gamepak_ram_pages = gamepak_ram_buffer_size / (32 * 1024);
  gamepak_memory_map = (gamepak_swap_entry_type *)malloc(sizeof(gamepak_swap_entry_type) * gamepak_ram_pages);
#endif
}
...
void init_memory()
{
  ...
  memset(io_registers,  0, IOMEM_SIZE);
  memset(oam_ram,       0, OAM_SIZE);
  memset(palette_ram,   0, PALETTE_SIZE);
  memset(iwram,         0, INTRAM_SIZE);
  memset(ewram,         0, WORKRAM_SIZE);
