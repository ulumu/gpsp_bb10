...
void print_register_usage()
{
  ...
  SLOG("ARM register usage (%lf%% ARM instructions):\n",
...
void function_cc step_debug(u32 pc, u32 cycles)
{
   ...
   (current_debug_state == STEP_RUN))
  {
    ...
    SLOG("%x instructions in, VCOUNT %d, cycles remaining: %d \n",
...
//        next_input = get_gui_input();
    ...
    switch(key)
    {
...
#ifdef STDIO_DEBUG
      case 'x':
        SLOG("break at PC (hex): ");
        scanf("%08x", &breakpoint_value);
        debug_off(PC_BREAKPOINT);
        break;

      case 'c':
        SLOG("break after N instructions (hex): ");
        scanf("%08x", &breakpoint_value);
        breakpoint_value -= 1;
      ...
      case 'f':
        SLOG("break after N instructions, skip in IRQ (hex): ");
        scanf("%08x", &breakpoint_value);
        breakpoint_value -= 1;
      ...
      case 'g':
        SLOG("break after N instructions (since start): ");
        scanf("%d", &breakpoint_value);
        debug_off(COUNTDOWN_BREAKPOINT_B);
        break;

      case 'v':
        SLOG("break at VCOUNT: ");
        scanf("%d", &breakpoint_value);
